...
| 测量位置           |       普通 AdamW 当前显存 |       分片 AdamW 当前显存 |                节省显存 |     降幅 |
| -------------- | ------------------: | ------------------: | ------------------: | -----: |
| 模型初始化后         | 13,124.8（12.82 GiB） | 13,124.8（12.82 GiB） |                   0 |     0% |
| 优化器 `step()` 前 | 26,137.7（25.53 GiB） | 26,137.7（25.53 GiB） |                   0 |     0% |
| 优化器 `step()` 后 | 52,130.3（50.91 GiB） | 39,329.6（38.41 GiB） | 12,800.7（12.50 GiB） | 24.56% |
| 整个训练步骤峰值       | 52,430.3（51.20 GiB） | 41,118.7（40.15 GiB） | 11,311.6（11.05 GiB） | 21.57% |

| 显存组成             |     普通 AdamW |     分片 AdamW |           差值 |     降幅 |
| ---------------- | -----------: | -----------: | -----------: | -----: |
| 模型参数             | 12,995.9 MiB | 12,995.9 MiB |            0 |     0% |
| 梯度               | 12,995.9 MiB | 12,995.9 MiB |            0 |     0% |
| 优化器状态            | 25,991.9 MiB | 13,191.2 MiB | 12,800.7 MiB | 49.25% |
| 其他开销（`step()` 后） |    146.3 MiB |    146.3 MiB |            0 |     0% |
| `step()` 后总显存    | 52,130.3 MiB | 39,329.6 MiB | 12,800.7 MiB | 24.56% |

不难看出，优化器分片后，优化器的状态显存减半了（2个gpu），当然不可能刚好减半，这取决于优化器状态的分片逻辑，整个训练步骤的峰值显存也降低了约 21.57%，还是有用的。

优化器后峰值显存下降了约25%，这很合理，因为按公式算出来差不多就是这个数值，设模型参数为 $\Psi$，梯度为参数也为$\Psi$，则普通 AdamW 的显存峰值为占据 $2\Psi $，而分片 AdamW 的显存峰值为 $\Psi$，所以峰值显存下降幅度为 25%，即优化器状态的一半。

| 指标     |         普通 AdamW |         分片 AdamW |                变化 |
| ------ | ---------------: | ---------------: | ----------------: |
| 平均每步时间 |     1,111.145 ms |     1,336.991 ms |     增加 225.846 ms |
| 相对耗时   |            1.00× |           1.203× |          慢 20.33% |
| 标准差    |        21.305 ms |        30.289 ms |       增加 8.984 ms |
| 中位数    |     1,098.749 ms |     1,347.118 ms |     增加 248.369 ms |
| P95    |     1,143.134 ms |     1,372.150 ms |     增加 229.016 ms |
| 吞吐量    | 3,686.3 tokens/s | 3,063.6 tokens/s | 下降 622.7 tokens/s |
| 吞吐量降幅  |                — |                — |            16.89% |

但是时间增加了，大概是引入了额外的通信开销。这也许可以进一步优化，比如批量通信什么的，但是优化空间不大。

和**ZeRO stage 1 (described as  ZeRO-DP Pos in S. Rajbhandari, J. Rasley, O. Ruwase, and Y. He [5])**的对比：
- 内存上，两者都差不多，都是把优化器状态分片到gpu上，zero可能分的更细致一点，我这个粗略分
